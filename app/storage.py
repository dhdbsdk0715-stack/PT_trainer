# app/storage.py — 파일/시트 I/O + 변환 유틸 (HOTFIX: 시트 자동 복구 추가)
import os
...
try:
    ...
    _ox_from_excel = None
# ------------- 내부 유틸 -------------
def _ensure_ws_exists_with_headers(wb):
    """기존 파일에도 필수 시트/헤더가 없으면 자동 생성/복구."""
    # exercises
    if S_EX not in wb.sheetnames:
        ws_ex = wb.create_sheet(S_EX)
        ws_ex.append(["부위", "운동명"])
        ...
        ws_ex = wb[S_EX]
        first = next(ws_ex.iter_rows(values_only=True), None)
        if not first or list(first)[:2] != ["부위","운동명"]:
            # 헤더 보정
            ws_ex.insert_rows(1)
            ws_ex.cell(row=1, column=1, value="부위")
            ws_ex.cell(row=1, column=2, value="운동명")
    # logs
    if S_LOG not in wb.sheetnames:
        ws_log = wb.create_sheet(S_LOG)
        ws_log.append(["날짜(YYYY-MM-DD)","닉네임","목표","분할","부위","운동명","세트수","반복수","무게(kg)","완료세트","총볼륨"])
    else:
        ws_log = wb[S_LOG]
        first = next(ws_log.iter_rows(values_only=True), None)
        if not first:
            ws_log.append(["날짜(YYYY-MM-DD)","닉네임","목표","분할","부위","운동명","세트수","반복수","무게(kg)","완료세트","총볼륨"])
    # profiles
    if S_PROF not in wb.sheetnames:
        ...
        ws_pf = wb[S_PROF]
        first = next(ws_pf.iter_rows(values_only=True), None)
        if not first:
            ws_pf.append(["닉네임","성별","키(cm)","몸무게(kg)","나이","목표","업데이트시각"])
    # meals
    if S_MEAL not in wb.sheetnames:
        ...
        ws_meal = wb[S_MEAL]
        first = next(ws_meal.iter_rows(values_only=True), None)
        if not first:
            ws_meal.append(["날짜(YYYY-MM-DD)","닉네임","식사분류","메뉴명","주성분","중량(g)","개수","횟수"])
# ------------- 초기화 -------------
def init_workbook():
    """파일이 없으면 생성. 있더라도 필수 시트가 없으면 자동 복구."""
    ...
    _ensure_ws_exists_with_headers(wb)
    wb.save(FILE_PATH)
def load_wb():
    # 안전: 호출 전후로 항상 보정
    init_workbook()
...
def read_exercises_by_part():
    ...
    wb = load_wb()
    if S_EX not in wb.sheetnames:
        _ensure_ws_exists_with_headers(wb); wb.save(FILE_PATH); wb = load_wb()
    ws = wb[S_EX]
    data, header = {}, True
